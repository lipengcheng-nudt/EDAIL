...
from rlf.storage import SimpleStorage, RolloutStorage
import time
import math
...
try:
    ...
    pass
from edail.ddpm import MLPConditionDiffusion
...
class Discriminator(nn.Module):
    ...
    def diffusion_loss(self, label, sa_pair, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, n_steps):
        ...
        if self.args.sample_strategy == "constant":
            ...
            t = torch.randint(0, self.n_steps, (batch_size,)).to(self.args.device)
...
class EDAIL(NestedAlgo):
    def __init__(self, agent_updater=PPO(), get_discrim=None):
        super().__init__([EDAILDiscrim(get_discrim, policy=agent_updater), agent_updater], 1)


class EDAILDiscrim(BaseIRLAlgo):
    def __init__(self, get_discrim=None, policy=None):
        super().__init__()
        if get_discrim is None:
            get_discrim = get_default_discrim  # 扩散判别器
    ...
    def _create_discrim(self):
        ...
        base_net = self.policy.get_base_net_fn(ob_shape)  # rlf.rl.model.PassThroughBase x=x output_shape=ob_shape
        #* Change to Diffusion Model
        discrim = self.get_discrim(base_net.output_shape[0], ac_dim, self.args, base_net,
                                   num_units=self.args.discrim_num_unit)  # discrim_num_unit default=128/yaml修改
    ...
    def init(self, policy, args):
        ...
        self.explore_storage = SimpleStorage(  #4096
            1024,
            1,
            rutils.get_obs_shape(self.policy.obs_space),
            rutils.get_ac_dim(self.action_space),
        )
        self.explore_storage.to(self.args.device)
        ...
        self.threshold = 0.9
        self.c = torch.tensor(self.args.c)
    ...
    def _get_sampler(self, storage, storage_name):
        batch_size = self.expert_train_loader.batch_size
        num_batches = len(self.expert_train_loader)
        if self.explore_storage.num_steps_can_sample() > num_batches * batch_size:
            self.allowed = True
        if storage_name == "expert":
            sampler = self.expert_train_loader
        elif storage_name == "fake_expert":
            if not self.allowed:
                sampler = self.expert_train_loader
            else:
                sampler = self.explore_storage.get_generator(None, mini_batch_size= batch_size)
        else:
            sampler = storage.get_generator(None, mini_batch_size=batch_size)
        return sampler
    ...
    def _compute_discrim_loss(self, agent_batch, expert_batch, explore_batch, obsfilt):
        ...
        expert_states = self._norm_expert_state(expert_batch['state'],
                                                obsfilt)
        if not self.allowed:
            explore_actions = explore_batch['actions'].to(self.args.device)
            explore_actions = self._adjust_action(explore_actions)
            explore_states = self._norm_expert_state(explore_batch['state'],
                                                     obsfilt)
        else:
            explore_states = explore_batch['state']
            explore_actions = explore_batch['action']
        ...
        explore_actions = rutils.get_ac_repr(
            self.action_space, explore_actions)
        expert_actions = rutils.get_ac_repr(
            self.action_space, expert_actions)
        expert_states_input = torch.cat([expert_states, explore_states], dim=0)
        expert_actions_input = torch.cat([expert_actions, explore_actions], dim=0)
        expert_d = self._compute_disc_val(expert_states_input, expert_actions_input)
    ...
    def _update_reward_func(self, storage, gradient_clip=False, t=1):
        ...
        expert_sampler = self._get_sampler(storage, "expert")
        fake_expert_sampler = self._get_sampler(storage, "fake_expert")
        agent_sampler = self._get_sampler(storage, "negative")
        ...
        for epoch_i in range(self.args.n_drail_epochs):
            ...
            for expert_batch, fake_expert_batch, agent_batch in zip(expert_sampler, fake_expert_sampler, agent_sampler):
                count += 1
                n += 1
                expert_d, agent_d, grad_pen = self._compute_discrim_loss(agent_batch, expert_batch, fake_expert_batch,
                                                                         obsfilt)
                expert_ds.append(expert_d[0:128])
        ...
        self.threshold = torch.cat(expert_ds).mean().clip(0.90,1).item()
        for k in log_vals:
            if k[0] != '_':
                log_vals[k] /= n
    ...
    def _compute_discrim_reward(self, storage, step, add_info):
        ...
        mask = (s >= self.threshold).squeeze()
        for st, ac in zip(state[mask], action[mask]):
            self.explore_storage.insert(st, ac, st)

        eps = 1e-20
        if self.args.reward_type == 'airl':
            reward = (s + eps).log() - (1 - s + eps).log() + self.c
        elif self.args.reward_type == 'gail':
            reward = (s + eps).log()
        elif self.args.reward_type == 'raw':
            reward = s
