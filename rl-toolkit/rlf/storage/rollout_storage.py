...
if __name__=="__main__":
    ...
    num_steps = 10
    num_processes = 2
    obs_space = torch.zeros(3,)  # 假设观测空间为 3 维
    action_space = torch.zeros(2)  # 假设动作空间为 2 维
    ...
    # 初始化存储
    obs = torch.randn(num_processes, 3)
    rollout_storage.init_storage(obs)

    # 插入模拟数据
    for step in range(num_steps):
        next_obs = torch.randn(num_processes, 3)
        rewards = torch.randn(num_processes, 1)
        done = torch.zeros(num_processes, 1, dtype=torch.bool)
        info = [{} for _ in range(num_processes)]
        ac_info = type("AcInfo", (), {
            "action": torch.randn(num_processes, 2),
            "take_action": torch.randn(num_processes, 2),
            "action_log_probs": torch.randn(num_processes, 1),
            "value": torch.randn(num_processes, 1),
            "hxs": {}
        })()
        rollout_storage.insert(obs, next_obs, rewards, done, info, ac_info)
        obs = next_obs

    # 使用生成器获取迷你批次数据
    advantages = rollout_storage.compute_advantages()
    generator = rollout_storage.get_generator(advantages, mini_batch_size=4)
