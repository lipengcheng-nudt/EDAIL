...
class RunSettings(MasterClass):
    ...
    def get_args(self, algo, policy):
        parser = self.get_parser()  # 获取默认参数
        algo.get_add_args(parser)  # 获取算法参数
        policy.get_add_args(parser)  # 获取策略参数
    ...
    def _sys_setup(self, add_args, ray_create, algo, policy):
        ...
        args = self.get_args(algo, policy)  # 获取参数(默认参数+算法参数+策略参数)
        ...
        if args.ray:
            ...
            if ray_create:
                return None, None
            log = self.get_logger()
        for k, v in vars(self.base_args).items():  # 合并参数和基础参数
            if k not in args:
                setattr(args, k, v)
        log.init(args)
        log.set_prefix(args)
    ...
    def create_runner(self, add_args={}, ray_create=False) -> rlf.Runner:
        ...
        """
        policy = self.get_policy() # ppo
        algo = self.get_algo()   # EDAIL()
        ...
        storage = algo.get_storage_buffer(policy, envs, args)  # RolloutStorage
